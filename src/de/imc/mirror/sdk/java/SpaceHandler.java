...
public class SpaceHandler implements de.imc.mirror.sdk.OfflineModeHandler, de.imc.mirror.sdk.SpaceHandler {
	private static final Logger logger = Logger.getLogger(SpaceHandler.class.getName());
	 ...
	 * @param dbConfig A configuration profile for the database to be used for offline storages.
	 */
	public SpaceHandler(ConnectionHandler connectionHandler, DatabaseConfig dbConfig) {
		if (connectionHandler == null || dbConfig == null){
		...
		datawrapper = DataWrapper.getInstance(dbConfig);
		...
		try {
			...
			if (response != null){
				...
				if ("result".equalsIgnoreCase(type)){
					...
					PrivateSpace defaultSpace = new de.imc.mirror.sdk.java.PrivateSpace("", spaceId, connectionHandler.getConfiguration().getDomain());
					...
					if (error.size() > 0) {
						logger.log(Level.SEVERE, "Spacehandler: Error", error.get(0).getText());
					}
		...
		try {
			...
			if (response != null){
				...
				if ("result".equalsIgnoreCase(type)){
					...
					String spaceId = elements.get(0).getAttributeValue("space");
					Space space = de.imc.mirror.sdk.java.Space.createSpace(config.getName(), 
							spaceId, 
	...
	private SpaceConfiguration parseSpaceConfiguration(Element configElement){
		...
		if (configElement == null || (fields = getChildren(configElement, "field")) == null || fields.size() == 0){
			...
			for (String member:members){
				spaceMembers.add(new de.imc.mirror.sdk.java.SpaceMember(member, Role.MEMBER));
			}
			for (String moderator:moderators){
				spaceMembers.add(new de.imc.mirror.sdk.java.SpaceMember(moderator, Role.MODERATOR));
			}
			SpaceConfiguration config = new de.imc.mirror.sdk.java.SpaceConfiguration(type,name, spaceMembers, persistenceType, persistenceDuration);
			return config;
		}
	}
	...
	private Space parseSpaceChannels(Element spaceInfo, Space space){
		...
		for (Element channel: channels){
			...
			channelsList.add(new de.imc.mirror.sdk.java.SpaceChannel(type, properties));
		}
		Space result = de.imc.mirror.sdk.java.Space.createSpace(
			space.getName(), 
			space.getId(), 
			space.getDomain(), 
			null, 
	...
	private List<Space> retrieveAvailableSpaces() throws SpaceManagementException{
		...
		if ("result".equalsIgnoreCase(element.getAttributeValue("type"))){
			...
			if (items != null){
				for (Element item:items){
					...
					spacesList.add(
						de.imc.mirror.sdk.java.Space.createSpace(
		...
		/*else if (iq.getType().equals("error")){
			logger.log(Level.SEVERE, "Spacehandler: Error", iq.error.text.text);
			throw new SpaceManagementException("An error response was received.",  SpaceManagementException.Type.OTHER);
		}*/
		return spacesList;
	}
	...
	private Space retrieveAllSpaceInformation(Space space) throws SpaceManagementException{
		...
		if (space.getType() == Space.Type.ORGA){
			try {
				...
				logger.log(Level.SEVERE, "A ConnectionStatusException was thrown while trying to retrieve the supported DataModels.", e);
			}
		}
		return de.imc.mirror.sdk.java.Space.createSpace(config.getName(), space.getId(), space.getDomain(), dataModels, config.getType(),
	...
	public OrgaSpace setModelsSupportedBySpace(String spaceId, final Set<de.imc.mirror.sdk.DataModel> dataModels) throws SpaceManagementException, ConnectionStatusException{
		...
		OrgaSpace orga = new de.imc.mirror.sdk.java.OrgaSpace(space.getName(), 
																spaceId +"", 
																space.getDomain(), 
																dataModels, 
																space.getChannels(),
	...
	public List<Space> getAllSpaces() {	
		...
		if (getMode() == Mode.ONLINE){
			...
			datawrapper.saveSpaces(result, userInfo.getBareJID());
				...
				logger.log(Level.SEVERE, "A SpaceManagementException was thrown while retrieving all spaces.", e);
			} 
		} else {
	...
	public Space getSpace(String spaceId) {
		if (getMode() == Mode.ONLINE) {
			...
			space = de.imc.mirror.sdk.java.Space.createSpace(null, 
																spaceId, 
																userInfo.getDomain(), 
																null, 
																type, 
			...
			try {
				...
				logger.log(Level.WARNING, "Failed to retrieve space information.", e);
				result = null;
			}
			return result;
		} else {
	...
	public Map<String, String> getSpacesList() {
		...
		if (getMode() == Mode.ONLINE){
			try {
				...
				logger.log(Level.SEVERE, "A SpaceManagementException was thrown while retrieving a list of all spaces.", e);
			}
		}
		spaces = datawrapper.getCachedSpacesForUser(userInfo.getBareJID());
		for (Space space:spaces){
